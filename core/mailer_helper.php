...
function sendOrderPlacedEmail($to, $customerName, $order_id, $items, $subtotal, $deliveryFee, $discount, $total, $paymentMethod, $address) 
{
  ...
  if (is_array($items)) {
    foreach ($items as $item) {
      ...
      $itemEmoji = (string)($item['food_emoji'] ?? ($item['emoji'] ?? '🍴'));
      // Remove FA icons from emoji string if present
      if (strpos($itemEmoji, '<i') !== false) $itemEmoji = '🍴';
...
function sendOrderCancelledByCustomerEmail($to, $customerName, $order_id)
{
  ...
  $content = "
      <h2 style='color:#d93025; margin:0 0 10px; font-size:24px;'>Order Cancelled 🗑️</h2>
...
function sendOrderStatusEmail($to, $customerName, $order_id, $status, $riderName = '')
{
  ...
  $statusConfig = [
    ...
    'out_for_delivery' => ['🏍️', 'Out for Delivery', 'Your order is on its way! Our delivery partner ' . ($riderName ?: 'is') . ' arriving soon.'],
